...
@onready var player_text 			= $spawn/player_custom/main/player/player_text
@onready var color_picker 			= $spawn/player_custom/main/player/mods/color_picker

@onready var player_name : LineEdit	= $spawn/player_custom/main/player_name

@onready var problem_team 			= $spawn/player_custom/main/team/problem_team
@onready var solver_team 			= $spawn/player_custom/main/team/solver_team

@onready var join_button 			= $spawn/player_custom/main/join
@onready var warning_message 		= $spawn/player_custom/main/warning_message

func _ready():
	join_button.disabled = true
	warning_message.visible = true
	
	color_picker.color = Color( randf_range(0,1), randf_range(0,1), randf_range(0,1) )
	player_text.modulate = color_picker.color
	
	warning_message.text = "Team not selected"
...
func get_player_data() -> Dictionary:
	var p_name := player_name.placeholder_text
	if not player_name.text.is_empty():
		if player_name.text.is_valid_filename():
			p_name = player_name.text
	var p_team := randi_range(1,2)
	if problem_team.button_pressed:
		p_team = Network.PROBLEM
	elif solver_team.button_pressed: 
		p_team = Network.SOLVERS
		
	return {Network.NAME:p_name, Network.TEAM:p_team, Network.COLOR:color_picker.get_pick_color(), Network.IS_PLAYING:false, Network.LATENCY:0.0}

func _on_color_picker_color_changed(color):
	player_text.modulate = color
	_force_update()
...
func _on_problem_team_pressed():
	if multiplayer.multiplayer_peer != null:
		...
		warning_message.text = "Not Connected to a server"
...
func _force_update():
	Network.update_player_data.rpc_id(1, get_player_data(), multiplayer.get_unique_id() ) ## Force on-the-fly update
